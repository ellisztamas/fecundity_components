...
traits <- c("mass", "frut", "seed", "tofu", "ffit", "tfit", "surv") # a list of the traits we could map
# parameters to input
ix <- traits[7] # pull out the name of the trait we are mapping here
set.seed(345) # random seed.
nperms <- 10000

path <- paste("rqtl_", ix, ".csv", sep="")
...
for(y in 1:length(permutations)){
  permutations[[y]] <- scantwo(cross, pheno.col = y+1, method = "hk", n.perm = nperms, n.cluster = 4, verbose = F, model = "binary")
  cat("Permutations completed for", names(permutations)[y], "after", round(((proc.time()-t0)[3] /3600),2),"hours.\n")
}
# store permutations for later
save(permutations, file= paste("permutations_", ix, ".csv", sep=""))
...
for(y in 1:length(stepwise)){
  stepwise[[y]] <- stepwiseqtl(cross, pheno.col=y+1, penalties=penalties[[y]], method="hk", model="binary",
...
save(stepwise, file=paste("stepwise_", ix, ".Rdata", sep=""))
