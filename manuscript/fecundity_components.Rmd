...
Based on these criteria, we identified 'pleiotropic' regions associated with multiple traits if they contained colocalising QTL for two or more traits, for those traits that were directly observed (number of fruits per plant, seed number per fruit, survival and seed mass).
...
#pdf("figures/correlation_summary.pdf", width = 8/2.54, height = 12/2.54)
...
figure \@ref(fig:correlations), supporting figure 2).
A small number of lines showed unusually high seed mass and low fecundity (supporting figure 2). To examine whether these lines unduly inflate correlation coefficients, we repeated the analyses for fecundity and offspring investment excluding lines with mean seed mass greater than 35µg. 
...
```{r qtl-pleiotropy, fig.width=16.9/2.54, fig.height=17/2.54, fig.cap="QTL for fecundity, seed mass, and survival. Lanes show QTL for number of fruits per reproductive plant (Fr/RP), number of seeds per fruit (Sd/fr), number of seeds per reproductive plant (Sd/RP), seed mass (Sd mass) and survival (Surv). Arrows indicate most-likely QTL position and the effect of the Swedish genotype (upward: increased phenotype; downward: decreased phenotype) in Italy (red, upper panels) and Sweden (blue, lower panels) in the 2010 (10) and 2011 (11) experiments respectively. Vertical bars show the 95\\% Bayesian credible intervals for QTL position. Open arrows show QTL with credible intervals wider than 15.2cM. Grey boxes indicate regions harbouring QTL with pleiotropic effects on two or more traits, excluding number of seeds per fruit (Q1-Q8)."}
#setEPS()
...
QTL for overall fecundity (number of seeds per reproductive plant) could be explained by QTL for individual components of fecundity. In Italy we identified a total of 12 distinct QTL for overall fecundity, of which four were detected in both years (figure \@ref(fig:qtl-pleiotropy); supplementary table 2).
All of these loci colocalised with QTL for either number of fruits per reproductive plant or number of seeds per fruit, and seven loci colocalised with QTL for both components of fecundity. 
...
All of these loci colocalised with QTL for either number of fruits per reproductive plant or number of seeds per fruit, and four colocalised with QTL for both components of fecundity.

Most QTL for number of seeds per seedling planted corresponded to a QTL detected for fruit number per seedling planted (figure \@ref(fig:fitness-qtl); supplementary table 3), but including information about seed output per fruit did result in some changes in the map positions of QTL for overall fitness.
...
Moreover, three QTL for overall fitness were detected in Italy in 2010 (chr. 3 18.0 cM; chr 5. 12.9) and one in Italy in 2011 (chr. 1 13.5 cM), which colocalised with a QTL for number of seeds per reproductive plant in the same year.
Finally, the position of the QTL for overall fitness detected at the start of chromosome 5 in Italy in 2010 was shifted by 4.5 cM when overall fitness was quantified as number seeds per seedling planted (12.0 cM) rather than as number of fruits per seedling planted (7.5 cM; figure \@ref(fig:fitness-qtl)).
...
In Italy, we identified two QTL for seed mass in 2010 and five in 2011 (figure \@ref(fig:qtl-pleiotropy), supplementary table 4).
...
QTL for components of fitness that could be resolved to within 15.2 cM tended to map to one of eight distinct regions of the linkage map (Q1-Q8, indicated in grey in figure \@ref(fig:qtl-pleiotropy)).
...
Meanwhile, five regions in Italy (Q1, Q2, Q4, Q6, Q7) and three regions in Sweden (Q2, Q8) included QTL for both number of fruits per reproductive plant and seed number per fruit.
...
The only well-defined seed-mass QTL that did not colocalise with QTL for at least one other trait is the locus detected on chromosome 4, 14.1 cM in Italy in 2011.
