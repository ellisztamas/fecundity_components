...
bibliography: /home/GMI/thomas.ellis/Dropbox/bibtex_files/tellis.bib
...
Local alleles at QTL for seed number per fruit were generally associated with increased fecundity. In Italy we detected five distinct, well-defined QTL, as well as two additional poorly-defined QTL with credible intervals longer than 15.2cM (fig. \@ref(fig:qtl-pleiotropy), table \@ref(tab:qtl-summary-table); see supporting tables 1-5 for full model details). In all cases the the non-local Swedish allele was associated with decreased seed number. In Sweden we found six distinct QTL for seed number per fruit. The local Swedish allele was associated with an increase in seed production at four of these loci and with a decrease at the remaining two loci.

QTL for overall fecundity could be explained by QTL for components of fecundity (fig. \@ref(fig:qtl-pleiotropy)). In Italy we detected a total of twelve distinct QTL for seed number per reproductive plant, and one further poorly-defined QTL. All of these loci colocalised with QTL for either fruit number per plant or seed number per fruit, of which four loci colocalised with both components of fecundity. In Sweden, we detected a total of eight QTL distinct QTL for seed number per reproductive plant, of which four could be identified to within 15.2cM. Six of these loci colocalised with well-defined QTL for either fruit number per plant or seed number per fruit. Of the two remaining loci, one colocalised with a poorly defined QTL for fruit number per fruit (chr. 1, 22.7cM), and the other did not overlap with any detectable QTL for fecundity components.
...
We compared QTL mapping results for overall fitness based on seed and fruit number with those obtained from analyses using fruit number only. We detected 13 QTL for seed number per planted seedling (12 in Italy, and 7 in Sweden; fig. \@ref(fig:fitness-qtl)), and four of these (one in in Italy and three in Sweden) did not overlap with QTL for fruit number per planted seedling identified in a previous analysis (chromosome 2, 45.6cM in Italy in 2011, chromosome 1, 22.7cM in Sweden in 2010, chromosome 3, 18.0cM in Sweden in 2011, and chromosome 4, 41-42cM in Sweden in both years; fig. \@ref(fig:fitness-qtl)). These four QTL for overall fitness were thus detected only when information about number of seeds per fruit was included in the estimate of fitness. However, it is worth noting that the QTL detected on chromosome 2 in Italy in 2010 is located rather close to a previously documented QTL for number of fruits per seedling (fig. \@ref(fig:fitness-qtl)). All four QTL overlapped with well-defined QTL for seed number per fruit (fig. \@ref(fig:qtl-pleiotropy)). 
