...
#csl: evolution.csl
abstract: 'Local adaptation may entail shifts in relative allocation to survival, growth and reproduction, but also an increase in the ability to acquire resources in the local environment, and thereby in the pool of resources to be allocated to different life-history functions. However, the relative importance of antagonistic pleiotropy and overall increased resource status in adaptive evolution is rarely examined. We examined the genetic basis of correlations among components of fitness by combining new data on number of seeds per fruit with previously published information on survival and fruit production in a field experiment in which a recombinant inbred line (RIL) population of Arabidopsis thaliana was planted at the native sites of the parental genotypes in Italy and Sweden in two years. The local genotype produced more seeds per fruit than did the non-local genotype at both sites, and adaptive differentiation between the two  populations was thus stronger than was previously shown based on survival and fruit number only. In the RIL population, genetic correlations between survival and fecundity (number of seeds produced per reproducing plant), and between fruit number and seed number per fruit were positive. Quantitative trait loci (QTL) for these traits tended to map to the same regions of the genome, and showed positive pleiotropic effects. By contrast, genetic correlations between fecundity and seed size were weak, and the pleiotropic effects of QTL for seed mass on fecundity varied in direction. The results indicate that adaptive differentiation between the two focal populations largely reflects the evolution of increased ability to acquire resources (“condition”) in the local environment, rather than shifts in the relative allocation to different life-history traits.'
editor_options: 
...
However, in almost all other cases where we observe pleiotropy the effects have been positive (fig. \@ref(qtl-pleiotropy)), so this is unlikely.
...
Despite the frequent occurence of QTL affecting multiple traits, we found much stronger evidence of pleiotropic effects in Italy compared to Sweden (fig. \@ref(qtl-pleiotropy)).
This might be due at least in part to increase environmental variation in Sweden that would make it more difficult to detect QTL there. 
...
However, the heritabilities of number of seed mass and seeds per fruit and seed number are as high or higher in Sweden than they are in Italy, so increased environmental variance cannot be the sole explanation.
...
Firstly, resolution in a mapping population derived from a cross is limited by the number of recombination breakpoints, and linkage between markers decays slowly.
...
Therefore, from an evolutionary perspective, it does not matter whether a QTL reflects a single pleiotropic locus, or multiple linked loci, because both hypotheses make near-identical predictions about how traits will coevolve.
...
We therefore adopted a heuristic approach by defining QTL detected for different traits to colocalise, and hence be pleiotropic, if the confidence intervals of QTL positions overlapped.
...
For example, we were not able to assess the contribution of 'poorly-defined' QTL whose position could not be resolved to within 15.2cM, which likely reflect a single causative locus of weak effect, or multiple loci spread across the chromosome (fig. \@ref(fig:qtl-pleiotropy)).
...
The results of this study show that variation in number of seeds per fruit makes an important contribution to adaptive differentiation in *A. thaliana* in addition to differentiation in number of fruits and survival.
