---
title: "Posotive correlations among components of fecundity boost estimates of local adaptation in *A. thaliana*"
author: "Tom Ellis"
...
abstract: "Insight into the processes governing adaptive differentiation among natural populations requires reliable estimates of the strength and direction of selection. However, estimates of selection and local adaptation typically consider only one or a few components of fitness and may therefore miss trade-offs expressed as conflicting selection acting through other components of fitness. We quantified local adaptation and examined its genetic basis by mapping quantitative trait loci (QTL) for fitness and its components in a population of recombinant inbred lines (RILs) derived from a cross between two locally adapted populations of Arabidopsis thaliana planted at the sites of the two source populations in two years. Specifically, we examined whether estimates of local adaptation were affected by considering number of seeds per fruit and not only fruit production when quantifying fecundity, and we tested for trade-offs between number of seeds per fruit and number of fruits per reproducing plant, between fecundity (total seed production per reproducing plant) and survival, and between fecundity and seed size. The local genotype had a greater advantage over the non-local genotype when information on seed number per fruit was included in estimates of fitness compared to when it was not, but the effect was with one exception not strong. In the RIL population, fruit number per reproducing plant and seed number per fruit were positively correlated genetically in all four site × year combinations examined, as were fecundity and survival in three of four site × year combinations. Seed size and number of seeds produced were significantly and negatively correlated only in Sweden in one year. Several QTL for number of fruits and number of seeds per fruit, and for overall fecundity and survival mapped to the same genomic regions, and in all but one case allelic effects were in the same direction. Two of four QTL for seed mass in Italy, and one of seven in Sweden showed antagonistic pleiotropic effects on fecundity. The results show that consideration of seed production per fruit may in some years substantially increase estimates of the strength of local adaptation. The predominance of positive genetic correlations among fitness components and of positive pleiotropic effects of fitness QTL suggest that local adaptation largely reflects the evolution of increased ability to acquire resources (“condition”), rather than shifts in the relative allocation to different life-history traits. "
---
...
```{r selection, fig.height=11/2.54, fig.cap="Selection coefficients for fitness components and fitness estimates between parental lines. Error bars show 95% confidence intervals. Positive values indicate selection favouring the local genotype, negative values selection favouring the non-local genotype."}
...
cortests2 <- list(
  cor.test(ril_tofu$mean_it2010[ril_mass$mean_it2010 < 35], ril_mass$mean_it2010[ril_mass$mean_it2010 < 35], method = 's'),
...
```{r import qtl data, eval=FALSE}
# Import R/qtl data on seed number, mass and total estimated fitness (seed number * total fruit number).
library("qtl", quietly = T)
massqtl <- read.cross("csv", "", "data_files/rqtl_mass.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
frutqtl <- read.cross("csv", "", "data_files/rqtl_frut.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
seedqtl <- read.cross("csv", "", "data_files/rqtl_seed.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
tofuqtl <- read.cross("csv", "", "data_files/rqtl_tofu.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
tfitqtl <- read.cross("csv", "", "data_files/rqtl_tfit.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
ffitqtl <- read.cross("csv", "", "data_files/rqtl_ffit.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
survqtl <- read.cross("csv", "", "data_files/rqtl_surv.csv", na.strings=NA, genotypes = c("a", "b"), alleles=c("It","Sw"))
...
# import ste objects containing stepwise QTL fitting results.
saveRDS(mass_stepwise, file = "output/stepwise_mass.rds")
saveRDS(frut_stepwise, file = "output/stepwise_frut.rds")
saveRDS(seed_stepwise, file = "output/stepwise_seed.rds")
saveRDS(tofu_stepwise, file = "output/stepwise_tofu.rds")
saveRDS(ffit_stepwise, file = "output/stepwise_ffit.rds")
saveRDS(tfit_stepwise, file = "output/stepwise_tfit.rds")
saveRDS(surv_stepwise, file = "output/stepwise_surv.rds")



x <- readRDS('output/stepwise_mass.rds')
fitqtl(massqtl, pheno.col=y+1,  qtl=mass_stepwise[[y]], formula=formula(mass_stepwise[[y]]),
                          method="hk", model="normal", covar=NULL, get.ests=T, dropone=T)


stepwise_models <- list(
  mass = readRDS('output/stepwise_mass.rds'),
  frut = readRDS('output/stepwise_mass.rds'),
  seed = readRDS('output/stepwise_mass.rds'),
  tofu = readRDS('output/stepwise_mass.rds'),
  ffit = readRDS('output/stepwise_mass.rds'),
  tfit = readRDS('output/stepwise_mass.rds'),
  surv = readRDS('output/stepwise_mass.rds')
)
...
```{r qtl-pleiotropy, eval=F, fig.cap="QTL for fitness components in each site-year combination. Lanes show QTL for fruit number per reproductive plant (Fruits/RP), seed number per fruit (Seeds/fr), seed number per reproductive plant (Seeds/RP), seed mass (Sd mass) and survival (Surv). Arrows indicate most-likely QTL position and the effect of the Swedish genotype (upward: increased phenotype; downward: decreased phenotype) in Italy (red) and Sweden (blue). Vertical bars show the 95% Bayesian credible intervals for QTL position. Open arrows show QTL with credible intervals >15.2cM. Labels on the right of chromosome indicate QTL with pleiotropic effects on components of fecundity (c), fecundity and survival (s) and fecundity and seed mass (m) in Italy (red) and Sweden (blue)."}
...
# pdf('figures/QTL_pleiotropy.pdf', width=25/2.56, height = 16.9/2.54)

ts <- 0.2
par(mfrow=c(2,1), mar=c(1,3,1,0)+0.1, oma=c(0,0,0,0), cex=0.8)
...
```{r fig:qtl-pleiotropy, eval=F, fig.cap="QTL for fitness components in each site-year combination. Lanes show QTL for fruit number per reproductive plant (Fruits/RP), seed number per fruit (Seeds/fr), seed number per reproductive plant (Seeds/RP), seed mass (Sd mass) and survival (Surv). Arrows indicate most-likely QTL position and the effect of the Swedish genotype (upward: increased phenotype; downward: decreased phenotype) in Italy (red) and Sweden (blue). Vertical bars show the 95% Bayesian credible intervals for QTL position. Open arrows show QTL with credible intervals >15.2cM. Labels on the right of chromosome indicate QTL with pleiotropic effects on components of fecundity (c), fecundity and survival (s) and fecundity and seed mass (m) in Italy (red) and Sweden (blue)."}
library(qtltools)
mloc <- read.table('data_files/genetic_map_agren_etal_2013.csv', sep = '\t', header=T)
...
# boxes$track <- match(boxes$chr, colnames(chr123$lane_margins)) # vector denoting which track to plot to.
# for(b in 1:nrow(boxes)) plotQTL_box(chr123, boxes$track[b], boxes$upper[b], boxes$lower[b], border = F, col='gray75', lwd=2)
...
plotQTL_lanes(chr123, lty=0)
# Lane labels
text(chr123$lane_margins[seq(3,21,4),], 10, c("Fruits/RP", "Seeds/fr", "Seeds/RP", "Sd mass", "Surv"), col=1, cex=0.6, srt=0, adj=c(0.5,0.5))
text(chr123$lane_centres, 2, c("10", "11"), cex=0.6, adj=c(0.5,0.5), srt=0, col='gray30')
text(chr123$lane_margins[seq(2,21,2),], 6, c("IT", "SW"), cex=0.6, adj=c(0.5,0.5), srt=0, col=c('red3','blue4'))
...
segments(chr123$lane_margins[seq(1,19,2),] + os, 4, 
         chr123$lane_margins[seq(3,21,2),] - os, 4, col=c('red3','blue4'), lwd=1)
# 
# xv1 <- as.vector(chr123$lane_margins[seq(2,21,4),])
# xv2 <- as.vector(chr123$lane_margins[seq(4,21,4),])
# for(i in 1:length(xv1)){
#   underlined(xv1[i], 6, "IT", cex=0.7, srt=0, col='red3')
#   underlined(xv2[i], 6, "SW", cex=0.7, srt=0, col='blue4')
# }
for(i in 1:4){
  plotQTL_qtl(chr123, frut_stepwise[[i]], frut_fit[[i]], fill_cutoff = 15.2, lane = i   , col=colvec[i])
  plotQTL_qtl(chr123, seed_stepwise[[i]], seed_fit[[i]], fill_cutoff = 15.2, lane = i+4 , col=colvec[i])
  plotQTL_qtl(chr123, tofu_stepwise[[i]], tofu_fit[[i]], fill_cutoff = 15.2, lane = i+8 , col=colvec[i])
  plotQTL_qtl(chr123, mass_stepwise[[i]], mass_fit[[i]], fill_cutoff = 15.2, lane = i+12, col=colvec[i])
  if(i != 3){ # no QTL for survival in 2010 in Sweden
  plotQTL_qtl(chr123, surv_stepwise[[i]], surv_fit[[i]], fill_cutoff = 15.2, lane = i+16,  col=colvec[i])
  }
}
# label pleiotropic regions
text(chr123$lane_margins[21, c(1,1,1,2,3,3)] + ts,
...
# boxes <- read.csv("../data_files/fitness_boxes.csv")
# boxes$track <- match(boxes$chr, colnames(chr45$lane_margins)) # vector denoting which track to plot to.
# for(b in 1:nrow(boxes)) plotQTL_box(chr45, boxes$track[b], boxes$upper[b], boxes$lower[b], border = F, col='gray75', lwd=2)

# Lanes dividers
segments(chr45$lane_margins, 0,
         chr45$lane_margins, rep(-chr45$track_lengths, each=21),
         lty=2, col='gray50')
# segments(chr45$lane_margins[seq(3,17,4),], 0,
#          chr45$lane_margins[seq(3,17,4),], rep(-chr45$track_lengths, each=4),
#          lty=1, col=1)
segments(chr45$lane_margins[seq(1,21,4),], 00,
         chr45$lane_margins[seq(1,21,4),], rep(-chr45$track_lengths, each=6),
         lty=1, col=1)
plotQTL_lanes(chr45, lty=0)
# Lane labels
text(chr45$lane_margins[seq(3,21,4),], 10, c("Fruits/RP", "Seeds/fr", "Seeds/RP","Sd mass", "Surv"), col=1, cex=0.6, srt=0, adj=c(0.5,0.5))
text(chr45$lane_centres, 2, c("10", "11"), cex=0.7, adj=c(0.5,0.5), srt=0, col='gray30')
text(chr45$lane_margins[seq(2,21,2),], 6, c("IT", "SW"), cex=0.6, adj=c(0.5,0.5), srt=0, col=c('red3','blue4'))
...
segments(chr45$lane_margins[seq(1,19,2),] + os, 4, 
         chr45$lane_margins[seq(3,21,2),] - os, 4, col=c('red3','blue4'), lwd=1)
# 
# xv1 <- as.vector(chr45$lane_margins[seq(2,21,4),])
# xv2 <- as.vector(chr45$lane_margins[seq(4,21,4),])
# for(i in 1:length(xv1)){
#   underlined(xv1[i], 6, "It", cex=0.7, srt=0, col='red3')
#   underlined(xv2[i], 6, "Sw", cex=0.7, srt=0, col='blue4')
# }
...
for(i in 1:4){
  plotQTL_qtl(chr45, frut_stepwise[[i]], frut_fit[[i]], fill_cutoff = 15.2, lane = i   , col=colvec[i])
  plotQTL_qtl(chr45, seed_stepwise[[i]], seed_fit[[i]], fill_cutoff = 15.2, lane = i+4 , col=colvec[i])
  plotQTL_qtl(chr45, tofu_stepwise[[i]], tofu_fit[[i]], fill_cutoff = 15.2, lane = i+8 , col=colvec[i])
  plotQTL_qtl(chr45, mass_stepwise[[i]], mass_fit[[i]], fill_cutoff = 15.2, lane = i+12, col=colvec[i])
  if(i != 3){ # no QTL for survival in 2010 in Sweden
  plotQTL_qtl(chr45, surv_stepwise[[i]], surv_fit[[i]], fill_cutoff = 15.2, lane = i+16, col=colvec[i])
  }
}
# label pleiotropic regions
text(chr45$lane_margins[21, c("5","5","5")] + ts,
...
```{r fig:qtl-fitness, eval=F}
library(qtltools)
mloc <- read.table('data_files/genetic_map_agren_etal_2013.csv', sep = '\t', header=T)
colvec=c("red3", "red3","blue4","blue4")

# pdf('../figures/QTL_fitness.pdf', width=25/2.56, height = 16.9/2.54)

par(mfrow=c(2,1), mar=c(1,3,1,0)+0.1, oma=c(0,0,0,0), cex=0.8)

chr123 <- plotQTL(1:3, mloc, nlanes = 16)
plotQTL_base(chr123, by=20, maxy = 10)
#text(35, -95, "Chromosome")
...
# Lanes dividers
segments(chr123$lane_margins, 0,
         chr123$lane_margins, rep(-chr123$track_lengths, each=17),
         lty=2, col='gray50')
segments(chr123$lane_margins[seq(1,17,4),], 0,
         chr123$lane_margins[seq(1,17,4),], rep(-chr123$track_lengths, each=5),
         lty=1, col=1)
plotQTL_lanes(chr123, lty=0)
# Lane labels
text(chr123$lane_margins[seq(3,17,4),], 10, c("Fruits/RP", "Seeds/RP", "Fruits/sdl", "Seeds/sdl"), col=1, cex=0.6, srt=0, adj=c(0.5,0.5))
text(chr123$lane_centres, 2, c("10", "11"), cex=0.6, adj=c(0.5,0.5), srt=0, col='gray30')
text(chr123$lane_margins[seq(2,17,2),], 6, c("IT", "SW"), cex=0.6, adj=c(0.5,0.5), srt=0, col=c('red3','blue4'))

os <- 0.2
segments(chr123$lane_margins[seq(1,15,2),] + os, 4, 
         chr123$lane_margins[seq(3,17,2),] - os, 4, col=c('red3','blue4'), lwd=1)
# xv1 <- as.vector(chr123$lane_margins[seq(2,17,4),])
# xv2 <- as.vector(chr123$lane_margins[seq(4,17,4),])
# for(i in 1:length(xv1)){
#   underlined(xv1[i], 6, "It", cex=0.7, srt=0, col='red3')
#   underlined(xv2[i], 6, "Sw", cex=0.7, srt=0, col='blue4')
# }
...
for(i in 1:4){
  plotQTL_qtl(chr123, frut_stepwise[[i]], frut_fit[[i]], fill_cutoff = 15.2, lane = i   , col=colvec[i], lwd=2)
  plotQTL_qtl(chr123, tofu_stepwise[[i]], tofu_fit[[i]], fill_cutoff = 15.2, lane = i+4 , col=colvec[i], lwd=2)
  plotQTL_qtl(chr123, ffit_stepwise[[i]], ffit_fit[[i]], fill_cutoff = 15.2, lane = i+8 , col=colvec[i], lwd=2)
  plotQTL_qtl(chr123, tfit_stepwise[[i]], tfit_fit[[i]], fill_cutoff = 15.2, lane = i+12, col=colvec[i], lwd=2)
}



chr45 <- plotQTL(c(4,5), mloc, nlanes = 16)
plotQTL_base(chr45, by=20, maxy = 10)
#text(35, -95, "Chromosome")
...
# Lanes dividers
segments(chr45$lane_margins, 0,
         chr45$lane_margins, rep(-chr45$track_lengths, each=17),
         lty=2, col='gray50')
# segments(chr45$lane_margins[seq(3,17,4),], 0,
#          chr45$lane_margins[seq(3,17,4),], rep(-chr45$track_lengths, each=4),
#          lty=1, col=1)
segments(chr45$lane_margins[seq(1,17,4),], 0,
         chr45$lane_margins[seq(1,17,4),], rep(-chr45$track_lengths, each=5),
         lty=1, col=1)
plotQTL_lanes(chr45, lty=0)
# Lane labels
text(chr45$lane_margins[seq(3,17,4),], 10, c("Fruits/RP", "Seeds/RP", "Fruits/sdl", "Seeds/sdl"), col=1, cex=0.6, srt=0, adj=c(0.5,0.5))
text(chr45$lane_centres, 2, c("10", "11"), cex=0.6, adj=c(0.5,0.5), srt=0, col='gray30')
text(chr45$lane_margins[seq(2,17,2),], 6, c("IT", "SW"), cex=0.6, adj=c(0.5,0.5), srt=0, col=c('red3','blue4'))

os <- 0.2
segments(chr45$lane_margins[seq(1,15,2),] + os, 4, 
         chr45$lane_margins[seq(3,17,2),] - os, 4, col=c('red3','blue4'), lwd=1)

# xv1 <- as.vector(chr45$lane_margins[seq(2,17,4),])
# xv2 <- as.vector(chr45$lane_margins[seq(4,17,4),])
# for(i in 1:length(xv1)){
#   underlined(xv1[i], 6, "It", cex=0.7, srt=0, col='red3')
#   underlined(xv2[i], 6, "Sw", cex=0.7, srt=0, col='blue4')
# }
...
colvec=c("red3", "red3","blue4","blue4")
for(i in 1:4){
  plotQTL_qtl(chr45, frut_stepwise[[i]], frut_fit[[i]], fill_cutoff = 15.2, lane = i   , col=colvec[i], lwd=2)
  plotQTL_qtl(chr45, tofu_stepwise[[i]], tofu_fit[[i]], fill_cutoff = 15.2, lane = i+4 , col=colvec[i], lwd=2)
  plotQTL_qtl(chr45, ffit_stepwise[[i]], ffit_fit[[i]], fill_cutoff = 15.2, lane = i+8 , col=colvec[i], lwd=2)
  plotQTL_qtl(chr45, tfit_stepwise[[i]], tfit_fit[[i]], fill_cutoff = 15.2, lane = i+12, col=colvec[i], lwd=2)
}
# dev.off()
```

```{r fig:mass-fecundity-qtl, eval=F}
library(qtltools)
mloc <- read.table('data_files/genetic_map_agren_etal_2013.csv', sep = '\t', header=T)
...
```{r fig:fecundity-fitness-qtl, eval=F}
library(qtltools)
mloc <- read.table('../data_files/genetic_map_agren_etal_2013.csv', sep = '\t', header=T)
...
```{r fig:cluster-positions, eval=FALSE}
library(qtltools)
mloc <- read.table('../data_files/genetic_map_agren_etal_2013.csv', sep = '\t', header=T)
