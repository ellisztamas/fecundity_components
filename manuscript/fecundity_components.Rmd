---
title: Positive correlations among components of fecundity boost estimates of local adaptation in *Arabidopsis thaliana*
author: "Tom Ellis"
...
## QTL mapping 
We mapped QTL for seed number per fruit, per reproductive plant and per seedling, as well as seed mass using the Multiple-QTL-Mapping (MQM) algorithm using the R/qtl package [@jansen1993interval; @Broman2003; @arends2010]. Mapping results for survival, fruit number per reproductive plant and fruit number per seedling using stepwise regression have been previously described by @agren_genetic_2013; we repeated mapping analysis for these traits using MQM to allow direct comparison.

We performed mapping based on RIL mean data for each site-year combination separately. We performed backwards elimination of cofactors spread across the genome at every fifth marker. We performed 10,000 permuations of the data to determine genome-wide 5% LOD-significance thresholds for inclusion of additive QTL and epistatic interactions [@doerge1996permutation; Broman2009]. We first applied a quantile normal transformation to phenotypes before model selection. Finally, we used ANOVA on untransformed data to fit the multiple QTL model and calculate effect sizes with respect to the Swedish allele, as well as the proportion of the total phenotypic variance among RILs explained by each locus, and the proportion of the difference between parents explained by QTL.
...
```{r tab:summary of QTL models, eval=F, eval=T, dependson='selection-bootstraps', qtl_time = file.info('R/format_QTL_models.R')}
# uses data imported from source("R/parents.R")
# Import QTL
source("R/format_QTL_models.R")

# get the differences between parental means in each expeirment.
parent_diffs <- data.frame(
  parents$delta[parents$site == "italy"  & parents$year == "2010"],
  parents$delta[parents$site == "italy"  & parents$year == "2011"],
  parents$delta[parents$site == "sweden" & parents$year == "2010"],
  parents$delta[parents$site == "sweden" & parents$year == "2011"])
...
qtl_tab <- qtl_tab[c(2,3,4,1,7,5,6),]
...
kable(qtl_tab, row.names = F, digits = 1,
      col.names = c("Trait",
                    ...
                    "n. QTL", "LOD", "PVE", "PDE",
                    "n. QTL", "LOD", "PVE", "PDE"),
      caption = "Summary of QTL models for fruit number per reproductive plant (Fruits/RP), seed number per fruit (Seeds/fr), seed number per reproductive plant (Seeds/RP), seed mass (Sd mass) and survival (Surv), showing number of QTL, LOD scores, percentage of variance explained (PVE) among RIL means, and percentage of the difference between parental means explained (PDE)") %>%
...
```{r qtl-pleiotropy, out.extra='angle=90', fig.width=26/2.54, fig.height=16.9/2.54, fig.cap="QTL for components of fitness. Lanes show individual QTL for fruit number per reproductive plant (Fruits/RP), seed number per fruit (Seeds/fr), seed number per reproductive plant (Seeds/RP), seed mass (Sd mass) and survival (Surv). Arrows indicate most-likely QTL position and the effect of the Swedish genotype (upward: increased phenotype; downward: decreased phenotype) in Italy (red) and Sweden (blue). Vertical bars show the 95\\% Bayesian credible intervals for QTL position. Open arrows show QTL with credible intervals 15.2cM. Labels on the right of chromosome indicate QTL with pleiotropic effects on components of fecundity (c), fecundity and survival (s) and fecundity and seed mass (m) in Italy (red) and Sweden (blue). Grey boxes indicate the range of colocalising QTL for a single trait across site-year combinations."}
